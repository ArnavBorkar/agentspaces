...
- shallow `asp doctor` findings, including cause, next-action text, and safe
  repair plans when available;
...
and doctor finding text, causes, next actions, and repair-plan text are scanned
for common token-shaped secrets such as `token=...`, `password=...`, `sk-...`,
and GitHub token prefixes.

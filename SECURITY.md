...
- **File handling**: asp reads/writes within the workspace root and creates sibling fork directories. Checkpoints capture untracked files — including secrets like `.env` — into the local `.asp/` store by design (that's the product); `.asp/` inherits your filesystem permissions. Add patterns to `capture.extra_excludes` to keep specific files out of checkpoints.
